...
load_dotenv()  # Opcional si docker ya inyecta las vars
...
SERVICE_ACCOUNT_FILE = os.getenv("GOOGLE_APPLICATION_CREDENTIALS", "/app/jsoncred.json")
...
async def entrypoint(ctx: agents.JobContext):
    print("[🔧] Entré al entrypoint correctamente")
    session = AgentSession(
        llm=openai.realtime.RealtimeModel(
            ...
            speed=1.0,   # 1.2 mejorrr  de 0.25 a 4.0
            voice="echo"#'echo', 'ash', 'ballad', 'coral', 'echo', 'sage', 'shimmer', and 'verse'
        ),
    ...
    print("[ENTRYPOINT] Conectando sesión...")

    await ctx.connect()
    print("[ENTRYPOINT] Iniciando sesión con agente...")
    ...
    print("[ENTRYPOINT] Generando mensaje de bienvenida...")
    ...
    print("[ENTRYPOINT] Ejecutando loop del agente...")
